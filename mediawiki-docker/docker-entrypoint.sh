...
MW_DIR="wiki"
MW_LOCAL_PATH="/var/www/html/${MW_DIR}"

cd /var/www/html
# FIXME: Keep php files out of the doc root.
ln -s /usr/src/mediawiki ${MW_DIR}
cd ./${MW_DIR}
...
if [ -d "$MEDIAWIKI_SHARED" ]; then
	...
	# ${MW_LOCAL_PATH}/extensions is not already a symbolic link, then replace it
	if [ -d "$MEDIAWIKI_SHARED/extensions" -a ! -h ${MW_LOCAL_PATH}/extensions ]; then
		echo >&2 "Found 'extensions' folder in data volume, creating symbolic link."
		rm -rf ${MW_LOCAL_PATH}/extensions
		ln -s "$MEDIAWIKI_SHARED/extensions" ${MW_LOCAL_PATH}/extensions
	fi

	# If a skins folder exists inside the shared directory, as long as
	# ${MW_LOCAL_PATH}/skins is not already a symbolic link, then replace it
	if [ -d "$MEDIAWIKI_SHARED/skins" -a ! -h ${MW_LOCAL_PATH}/skins ]; then
		echo >&2 "Found 'skins' folder in data volume, creating symbolic link."
		rm -rf ${MW_LOCAL_PATH}/skins
		ln -s "$MEDIAWIKI_SHARED/skins" ${MW_LOCAL_PATH}/skins
	fi

	# If a vendor folder exists inside the shared directory, as long as
	# ${MW_LOCAL_PATH}/vendor is not already a symbolic link, then replace it
	if [ -d "$MEDIAWIKI_SHARED/vendor" -a ! -h ${MW_LOCAL_PATH}/vendor ]; then
		echo >&2 "Found 'vendor' folder in data volume, creating symbolic link."
		rm -rf ${MW_LOCAL_PATH}/vendor
		ln -s "$MEDIAWIKI_SHARED/vendor" ${MW_LOCAL_PATH}/vendor
	fi
